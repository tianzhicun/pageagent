...
SGH','Wapaka','DoCoMo','iPhone','iPod');
     ...
     */
    public static function downImg($url, $filePath = null,$openid,$actid,$fileName = null)
    {
        if (! $filePath) {
        ...
        $datapath = $file ? $urlPath . $fileName : false;
        return $datapath;
    }
     ...
     * @param string            服务器地址
     */
    public static function genQRcode($data, $proid,$fileName = null, $path = '/data/code/')
    {
        $urlPath = $path . '/';
        ...
        \QRcode::png($data, $qrcode, $errorCorrectionLevel, $matrixPointSize, 2);
        $pathcode = $urlPath.$fileName;
        //$pathresult = substr($pathcode,24);
        return $pathcode;
    }
     ...
     * @return string|boolean   服务器地址
     */
    public static function mixImgs(array $imgs, $filePath,$actid = NULL,$openid)
    { 
        if (is_array($imgs) && sizeof($imgs) > 1) {
            ...
            $file = imagecreatefromstring(file_get_contents($baseImg,false,$context));
            $length = sizeof($imgs);
            for ($i = 1; $i < $length; $i ++) {
                if($i == 2){
                    ...
                    list ($return_code, $return_content) = CommonTools::http_request_get($path);
                    $img = imagecreatefromstring($return_content);
                }else{
                    if (($path = self::getImgPath($imgs[$i]['src'])) === false) {
                        return false;
                    }
                    $img = imagecreatefromstring(file_get_contents($path,false,$context));
                }
            ...
            if($actid){
                $urlPath = $filePath . $actid . '/';
            }else{
                $urlPath = $filePath . date('Y') . '/' . date('md') . '/';
            }
            $filePath = $_SERVER['DOCUMENT_ROOT'] . $urlPath;
            if($openid){
                $output = $openid. $actid.'.jpg';
            }else{
            ...
            imagejpeg($file, $fileName,70);
            $fullpathname = $urlPath . $output;
            return $fullpathname;
        } else {
            return false;
        }
